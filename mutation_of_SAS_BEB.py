...
# Load the sample map dynamically
sample_map = pd.read_csv(tsv_file, sep="\t", header=None)

# Extract samples that contain "SAS___BEB"
sas_beb_samples = {}
for index, row in sample_map.iterrows():
    sample_id = row[0]  # First column = sample name
    populations = row[1:].dropna().astype(str)  # Other columns contain population info

    for pop_list in populations:
        pop_list = pop_list.split(",")  # Split multiple populations in a single cell
        for pop in pop_list:
            pop = pop.strip()
            if "SAS___BEB" in pop:
                if sample_id not in sas_beb_samples:
                    sas_beb_samples[sample_id] = []
                sas_beb_samples[sample_id].append(pop)
...
# Save results
output_file = os.path.join(output_dir, f"{gene_name}_SAS_BEB_samples.tsv")
...
# Save mutation results to a file
output_file = os.path.join(output_dir, f"{gene_name}_mutations.tsv")
with open(output_file, "w") as f:
    f.write(f"Gene with lowest GDR (SAS_BEB): {gene_name} ({gdr_value})\n")
    f.write(f"Using sample map file: {tsv_file}\n")
